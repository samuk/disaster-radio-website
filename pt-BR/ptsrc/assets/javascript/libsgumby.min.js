!function () {
    ...
    function t() {
        ...
        var t = $('script[gumby-touch]').attr('gumby-touch'), i = $('script[gumby-path]').attr('gumby-path');
    ...
    t.prototype.init = function (t) {
        ...
        for (var t = 0; t < arguments.length; t++) {
            var i = arguments[t], e = 'data-' + arguments[t], n = 'gumby-' + arguments[t];
            if (this.is('[' + e + ']'))
                return this.attr(e) ? this.attr(e) : !0;
            if (this.is('[' + n + ']'))
                return this.attr(n) ? this.attr(n) : !0;
            if (this.is('[' + i + ']'))
                return this.attr(i) ? this.attr(i) : !0;
        }
        return !1;
        ...
        if ('object' == typeof t) {
            var e = 0;
            for (e; e < t.length; e++)
                this.inits[t[e]] && 'function' == typeof this.inits[t[e]] ? this.inits[t[e]](i) : this.error('Error initializing module: ' + t[e]);
        } else
            this.inits[t] && 'function' == typeof this.inits[t] ? this.inits[t](i) : this.error('Error initializing module: ' + t);
        return this;
        ...
        t && (n = t);
        for (i in n)
            e = t ? n[i] : i, this.uiModules[e].init();
    }, window.Gumby = new t();
}(), !function () {
    'use strict';
    function t(t) {
    ...
    t.prototype.setup = function () {
        ...
        var t = $(window).scrollTop(), i = this.fixedPoint instanceof jQuery ? this.fixedPoint.offset().top : this.fixedPoint, e = !1;
        ...
        Gumby.debug('Element has been fixed', this.$el), Gumby.debug('Triggering onFixed event', this.$el), this.state = 'fixed', this.$el.css({ top: 0 + this.top }).addClass('fixed').removeClass('unfixed pinned').trigger('gumby.onFixed'), this.$parent && this.constrain();
        ...
        Gumby.debug('Element has been pinned', this.$el), Gumby.debug('Triggering onPinned event', this.$el), this.state = 'pinned', this.$el.css({ top: this.$el.offset().top }).addClass('pinned fixed').removeClass('unfixed').trigger('gumby.onPinned');
    ...
    t.prototype.update = function () {
        if (!(this.$el.hasClass('checked') && this.$input.prop('checked') && this.$el.find('i.icon-dot').length)) {
            ...
            var t = this.$el.find('span'), i = 'input[name="' + this.$input.attr('name') + '"]';
    ...
    t.prototype.fetchRetinaImage = function () {
        var t = this.imageSrc, i = this.imageSrc.search(/(\.|\/)(gif|jpe?g|png)$/i);
    ...
    t.prototype.setup = function () {
        ...
        if ('top' == e)
            ...
            this.targetPos = Number(e);
        else {
            if (i = $(e), !i.length)
                return Gumby.error('Cannot find skiplink target: ' + e), !1;
        ...
        var t = this;
        $('html,body').animate({ scrollTop: this.calculateOffset() }, this.duration, this.easing).promise().done(function () {
            Gumby.debug('Triggering onComplete event', t.$el), t.$el.trigger('gumby.onComplete');
        });
    }, t.prototype.calculateOffset = function () {
        if (!this.offset)
            return this.targetPos;
        var t = this.offset.substr(0, 1), i = Number(this.offset.substr(1, this.offset.length));
    ...
    t.prototype.init = function () {
        ...
        var t = Gumby.selectAttr.apply(this.$el, ['trigger']), i = 0, e = [];
        return t ? (i = t.indexOf('|'), -1 === i ? this.checkTargets([t]) ? [$(t)] : !1 : (e = t.split('|'), this.checkTargets(e) ? e.length > 1 ? [
            $(e[0]),
            $(e[1])
        ] : [$(e[0])] : !1)) : !1;
        ...
        for (i; i < t.length; i++)
            if (t[i] && !$(t[i]).length)
                return Gumby.error('Cannot find ' + this.constructor.name + ' target: ' + t[i]), !1;
        return !0;
    ...
    i.prototype.validate = function () {
        ...
        return this.each(function () {
            if (!n.required.length)
                return !1;
            var e, o = t(this), a = n.required.length;
            for (e = 0; a > e; e++)
                s.push(new i(o.find('[name="' + n.required[e].name + '"]'), n.required[e].validate || !1));
            o.on('submit', function (t) {
                ...
                if (!o.data('passed')) {
                    ...
                    for (e = 0; a > e; e++)
                        s[e].validate() || (i = !0);
                    if (i) {
                        if (n.fail && 'function' == typeof n.fail)
                            return n.fail(), void 0;
                    } else {
                        if (n.submit && 'function' == typeof n.submit)
                            return n.submit(o.serializeArray()), void 0;
...
}(jQuery), Gumby.autoInit && (Gumby.debug('Gumby auto initialization'), window.Gumby.init()), 'function' == typeof define && define.amd && define(window.Gumby), Gumby.touchEvents && (Gumby.debug('Loading jQuery mobile touch events'), Modernizr.load({
    test: Modernizr.touch,
    yep: Gumby.touchEvents + '/jquery.mobile.custom.min.js',
    callback: function () {
        return $.mobile ? (window.Gumby.click += ' tap', void 0) : (Gumby.error('Error loading jQuery mobile touch events'), void 0);
    }
}));