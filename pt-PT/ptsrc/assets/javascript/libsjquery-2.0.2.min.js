        ...
        var t, n, r, i, o, s, a, u, l, c, p, f, h, d, g, m, y, v = 'sizzle' + -new Date(), b = e.document, w = 0, T = 0, C = at(), k = at(), N = at(), E = !1, S = function () {
                ...
                for (; n > t; t++)
                    if (this[t] === e)
                        return t;
                return -1;
            }, R = 'checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped', M = '[\ \\t\\r\\n\\f]', W = '(?:\\\\.|[\\w-]|[^\ -\ ])+', $ = W.replace('w', 'w#'), B = '\\[' + M + '*(' + W + ')' + M + '*(?:([*^$|!~]?=)' + M + '*(?:([\'"])((?:\\\\.|[^\\\\])*?)\\3|(' + $ + ')|)|)' + M + '*\\]', I = ':(' + W + ')(?:\\((([\'"])((?:\\\\.|[^\\\\])*?)\\3|((?:\\\\.|[^\\\\()[\\]]|' + B.replace(3, 8) + ')*)|.*)\\)|)', z = RegExp('^' + M + '+|((?:^|[^\\\\])(?:\\\\.)*)' + M + '+$', 'g'), _ = RegExp('^' + M + '*,' + M + '*'), X = RegExp('^' + M + '*([>+~]|' + M + ')' + M + '*'), U = RegExp(M + '*[+~]'), Y = RegExp('=' + M + '*([^\\]\'"]*)' + M + '*\\]', 'g'), V = RegExp(I), G = RegExp('^' + $ + '$'), J = {
                ID: RegExp('^#(' + W + ')'),
                CLASS: RegExp('^\\.(' + W + ')'),
